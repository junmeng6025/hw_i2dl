...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # pass
        # img_got= []
        # for img_path in self.images[index]:
        #     img_np = self.load_image_as_numpy(img_path)
        #     img_got.append(img_np)

        img_got= self.load_image_as_numpy(self.images[index])

        if self.transform is not None:
            img_got = self.transform(img_got)

        data_dict = {"image":img_got, "label": self.labels[index]}
