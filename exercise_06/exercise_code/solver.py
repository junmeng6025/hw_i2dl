...
class Solver(object):
    ...
    def train(self, epochs=100, patience = None):
        ...
        for t in range(epochs):
            ...
            # --------------------------------------------------
            if t == 0:
                self.init_train_loss = train_epoch_loss
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            if t == 0:
                self.init_val_loss = val_epoch_loss
            # --------------------------------------------------
    ...
    def update_best_loss(self, val_loss, train_loss):
        ...
        if not self.best_model_stats or val_loss < self.best_model_stats["val_loss"]:
            ...
            # -------------------------------------------------------------------------------------------------
            # SELF-MODIFIED: end early the search, whose loss gets stuck in the initial status
            if abs(val_loss - self.init_val_loss) < 0.1 or abs(train_loss - self.init_train_loss) < 0.1:
